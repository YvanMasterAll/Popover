...
open class Popover: UIViewController {
    ...
    fileprivate var wrapperView         : UIView        = UIView(frame: .zero)
    ...
    open func show() {
        ...
        if child != nil, let _sender = child.popover.sender {
            sender = _sender
            wrapperView.frame = .init(origin: .zero, size: child.popover.contentSize)
            po_addchild(child, inview: wrapperView)
            ...
            sender = _sender
            wrapperView.frame = .init(origin: .zero, size: contentView.popover.contentSize)
        ...
        case .dialog_default:
            self.arrowSize = .zero
            self.arrowShowPoint = CGPoint(x: view.center.x,
                                          y: view.center.y - contentView.frame.height/2)
        }
        //hightlight effect for sender
        if self.highlightFromView {
            self.createHighlightLayer(fromView: sender, inView: view)
        }
        //overlay layer
        self.showOverlay()
    ...
    @objc open func _dismiss() {
        ...
        UIView.animate(withDuration: self.animationOut, delay: 0,
                       ...
                       animations: {
                        self.wrapperView.transform = CGAffineTransform(scaleX: 0.0001, y: 0.0001)
...
extension Popover {
    ...
    open func caculateLocation() {
        ...
        switch popoverType {
        case .up, .down, .auto:
            self.wrapperView.frame.origin.x = self.arrowShowPoint.x - self.wrapperView.frame.size.width * 0.5
            
            var sideEdge: CGFloat = 0.0
            if self.wrapperView.frame.size.width < self.view.frame.size.width {
                sideEdge = self.sideEdge
            }
            
            let outerSideEdge = self.wrapperView.frame.maxX - self.view.bounds.size.width
            if outerSideEdge > 0 {
                self.wrapperView.frame.origin.x -= (outerSideEdge + sideEdge)
            } else {
                if self.wrapperView.frame.minX < 0 {
                    self.wrapperView.frame.origin.x += abs(self.wrapperView.frame.minX) + sideEdge
                }
            }
        case .left, .right:
            self.wrapperView.frame.origin.y = self.arrowShowPoint.y - self.wrapperView.frame.size.height * 0.5
            
            var sideEdge: CGFloat = 0.0
            if self.wrapperView.frame.size.height < self.view.frame.size.height {
                sideEdge = self.sideEdge
            }
            
            let outerSideEdge = self.wrapperView.frame.maxY - self.view.bounds.size.height
            if outerSideEdge > 0 {
                self.wrapperView.frame.origin.y -= (outerSideEdge + sideEdge)
            } else {
                if self.wrapperView.frame.minY < 0 {
                    self.wrapperView.frame.origin.y += abs(self.wrapperView.frame.minY) + sideEdge
                }
            }
        }
        
        let arrowPoint = self.view.convert(self.arrowShowPoint, to: self.wrapperView)
        var anchorPoint: CGPoint
        switch self.popoverType {
        case .up:
            self.wrapperView.frame.origin.y = self.arrowShowPoint.y - self.wrapperView.frame.height - self.arrowSize.height
            anchorPoint = CGPoint(x: arrowPoint.x / self.wrapperView.frame.size.width, y: 1)
        case .down, .auto:
            self.wrapperView.frame.origin.y = self.arrowShowPoint.y
            anchorPoint = CGPoint(x: arrowPoint.x / self.wrapperView.frame.size.width, y: 0)
        case .left:
            self.wrapperView.frame.origin.x = self.arrowShowPoint.x - self.wrapperView.frame.width - self.arrowSize.height
            anchorPoint = CGPoint(x: 1, y: arrowPoint.y / self.wrapperView.frame.size.height)
        case .right:
            self.wrapperView.frame.origin.x = self.arrowShowPoint.x
            anchorPoint = CGPoint(x: 0, y: arrowPoint.y / self.wrapperView.frame.size.height)
        }
        ...
        let lastAnchor = self.wrapperView.layer.anchorPoint
        self.wrapperView.layer.anchorPoint = anchorPoint
        let x = self.wrapperView.layer.position.x + (anchorPoint.x - lastAnchor.x) * self.wrapperView.layer.bounds.size.width
        let y = self.wrapperView.layer.position.y + (anchorPoint.y - lastAnchor.y) * self.wrapperView.layer.bounds.size.height
        self.wrapperView.layer.position = CGPoint(x: x, y: y)
        //adjust location to show arrow point
        switch popoverType {
        case .up, .down, .auto:
            self.wrapperView.frame.size.height += self.arrowSize.height
            if self.wrapperView.frame.size.height != self.contentView.frame.size.height {
                self.contentView.frame.size.height += self.arrowSize.height
            }
        case .left, .right:
            self.wrapperView.frame.size.width += self.arrowSize.height
            if self.wrapperView.frame.size.width != self.contentView.frame.size.width {
    ...
    open func getArrowShowPoint() -> CGPoint {
        ...
        if self.popoverType == .auto {
            if let point = sender.superview?.convert(sender.frame.origin, to: nil),
                point.y + sender.frame.height + self.arrowSize.height + wrapperView.frame.height > view.frame.height {
                self.popoverType = .up
            } else {
...
extension Popover {
    ...
    open func showOverlay() {
        if self.dismissOnBlackOverlayTap || self.showBlackOverlay {
            ...
            view.insertSubview(self.blackOverlay, at: 0)
...
extension Popover {
    ...
    open func applyAnimation() {
        self.wrapperView.transform = CGAffineTransform(scaleX: 0.0, y: 0.0)
...
extension Popover {
    ...
    fileprivate func drawBackgroundLayerWithArrowPoint() {
        ...
        self.wrapperView.layer.insertSublayer(backgroundLayer, at: 0)
        self.wrapperView.layer.mask = backgroundLayer
    }
    ...
    func getArrowPointPath() -> UIBezierPath {
        ...
        let arrowPoint = self.view.convert(self.arrowShowPoint, to: self.contentView)
        switch self.popoverType {
        case .up:
    ...
    var isCornerLeftArrow: Bool {
        return self.arrowShowPoint.x == self.wrapperView.frame.origin.x
    }
    
    var isCornerRightArrow: Bool {
        return self.arrowShowPoint.x == self.wrapperView.frame.origin.x + self.wrapperView.bounds.width
    }
    
    var isCornerUpArrow: Bool {
        return self.arrowShowPoint.y == self.wrapperView.frame.origin.y
    }
    
    var isCornerDownArrow: Bool {
        return self.arrowShowPoint.y == self.wrapperView.frame.origin.y + self.wrapperView.bounds.height
    }
}
...
extension UIViewController {
    ...
    func po_present(_ vc: UIViewController, animated: Bool, completion: (() -> Void)?) {
        let popover = Popover()
        popover.child = vc
        popover.view.backgroundColor = UIColor(red: 0, green: 0, blue: 0, alpha: 0)
        popover.modalPresentationStyle = .overCurrentContext
        popover.modalTransitionStyle = .coverVertical
        self.present(popover, animated: animated, completion: completion)
    }
    
    func po_present(_ tv: UIView, animated: Bool, completion: (() -> Void)?) {
        let popover = Popover()
